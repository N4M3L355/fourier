// comment to stop showing duplicates in webstorm
//
// let out = (where => (...what) => {
...
//     return false;
//   });
//   document.getElementById('brushSelect').addEventListener('change',  x=>{
//     config.setup.currentOutput = x.target.value;
//   });
//
...
//             });
//             return containers;
//             containers = containers.map((x,i) => ({note: myScale[i], amp: x}))
...
//           fx: v => {
//             s.fill(30, 100, 100-v*100);
//             //s.ellipse(x, y, (1 - Math.abs(1 - 2 * v)) ** 2 * 100)
//             s.ellipse(x,y,50-50*v);
//           }
//         }),
//         path: (x,y) => ({
//           name: "path",
...
//     config.outputs.push({   //midScreenSpectrum
...
//       output.fx(output.input(spectrum,positions));
//     });
//
//
//     objects.forEach((x,i) => {
...
//
//     if (s.mouseIsPressed) {
//       config.outputs.push(config.brushes[config.setup.currentOutput](s.mouseX, s.mouseY));
//     }
//     s.stroke(255);
